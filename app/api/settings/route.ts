...
export async function GET(request: Request) {
  ...
  if (profile.company_id) {
    const full = await db.from('companies')
      .select('id, name, type, contact_email, phone, address, license_number, default_payment_terms, default_billing_mode, default_contract_type, default_retainage_pct, auto_logout_minutes, logo_url, delete_protection_enabled, delete_key_hash')
...
export async function PATCH(request: Request) {
  ...
  if (company && typeof company === 'object') {
    ...
    if (profile?.company_id) {
      ...
      const allowed = ['name', 'type', 'contact_email', 'phone', 'address', 'license_number', 'default_payment_terms', 'default_billing_mode', 'default_contract_type', 'default_retainage_pct', 'auto_logout_minutes']
      ...
      if (Object.keys(companyUpdates).length > 0) {
        ...
        if (error && (error as any).code === '42703') {
          const { default_billing_mode: _b, default_contract_type: _c, default_retainage_pct: _r, auto_logout_minutes: _a, ...rest } = companyUpdates
