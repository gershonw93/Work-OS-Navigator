...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    lead: 'A schedule is only worth keeping if it survives the first delay. Framing runs three days long, and the sheetrock guy, the painter and the trim carpenter all need telling - which is four phone calls nobody makes, so the schedule quietly stops matching the job.',
    without: 'The old way: you re-type five sets of dates, forget one, and find out on site. Or you leave the schedule alone because updating it is worse than working around it.',
    youDo: 'You move one date - or press "Running late?" and say what happened.',
    auto: [
      { text: 'Every trade waiting on that one shifts by the same number of days, all the way down the chain', who: ['You', 'Office'] },
      { text: 'A review screen shows what moves, what does not and WHY, before a single date is written', who: ['You'] },
      { text: 'You choose: email the affected subs, or shift the dates quietly. Nothing is sent unless you press it', who: ['You', 'Sub'] },
      { text: 'Each sub gets ONE email listing all of their dates that moved, and which trade pushed them', who: ['Sub'] },
      { text: 'Drop a pin on the exact spot on the drawing and it becomes a task: assignee, due date, done', who: ['Field crew', 'Site manager'] },
    ],
    details: [
      { icon: Layers, title: 'Say what waits for what, once', body: 'Pick the trade a line comes after. You can point at a trade you have not awarded yet - add a placeholder, link to it, and when you award the real sub SyteNav asks whether that new line takes its place.' },
      { icon: ShieldCheck, title: 'Hold a trade until the one ahead is far enough along', body: 'Not "after framing finishes" but "after framing hits 80%". When it clears, SyteNav offers to tell the sub they are clear to start - again, only if you press it.' },
      { icon: ClipboardList, title: 'Pin the blueprint, skip the ambiguity', body: 'Instead of "the outlet by the kitchen, you know the one," drop a pin on the exact spot on the plan. The task carries the location with it, and the crew taps the pin to see what is needed.' },
      { icon: CalendarDays, title: 'A record of every slip', body: 'A delay is logged with the reason and what the dates were before it, so as-planned against as-built is a question the job can answer months later.' },
    ],
    wide: true,
