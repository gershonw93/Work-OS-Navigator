...
import { DollarSign, TrendingUp, Clock, CheckCircle2, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function FinancialsPage({ params }: { params: { id: string } }) {
  ...
  const totalCommitted = data.total_contracted + data.approved_change_orders
  ...
  const statCards = [
    ...
    { label: 'Outstanding', value: totalOutstanding, color: 'text-amber-700', bg: 'bg-amber-50', icon: Clock, desc: 'Approved + sent invoices' },
  ]
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Subcontracts breakdown */}
      {data.subcontracts.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100">
            <h2 className="text-sm font-semibold text-slate-700">Subcontracts</h2>
          </div>
          <div className="divide-y divide-slate-50">
            {data.subcontracts.map((sub: any) => {
              ...
              return (
                <div key={sub.id}>
                  <button className="w-full flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 transition-colors text-left"
          ...
          <div className="divide-y divide-slate-50">
            {data.change_orders.map((co: any) => (
              <div key={co.id} className="flex items-center gap-4 px-5 py-3 text-sm">
                ...
                <span className={cn('text-xs font-medium rounded-full border px-2 py-0.5',
                  co.status === 'answered' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-slate-50 border-slate-200 text-slate-600')}>
                  {co.status}
                </span>
