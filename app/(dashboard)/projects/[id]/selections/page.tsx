...
import {
  SELECTION_CATEGORIES, SELECTION_STATUSES, STATUS_TINT,
      ...
      {rows.length === 0 ? (
        <div className="bg-panel rounded-xl border border-line p-10 text-center">
          ...
          <p className="text-sm text-muted-fg max-w-md mx-auto">
            No selections yet. Start from the standard list for a house - windows, roofing, cabinets,
            fixtures, paint, flooring, tile - each with a typical lead time already on it. Delete what
            doesn&apos;t apply.
          </p>
          <Button onClick={seed} disabled={seeding} className="mt-4 gap-1.5">
            {seeding ? <><Loader2 className="h-4 w-4 animate-spin" /> Building…</> : <><Sparkles className="h-4 w-4" /> Start the standard board</>}
          </Button>
        </div>
      ) : (
        <>
