...
export default function DailyLogsPage({ params }: { params: { id: string } }) {
  ...
  async function handleCreateTask(e: React.FormEvent) {
    ...
    await fetch(`/api/projects/${params.id}/tasks`, {
      ...
      body: JSON.stringify({
        title: taskTitle,
        description: taskDescription || `From daily log ${formatDate(createTaskFromLog.log_date)}: ${createTaskFromLog.issue_description ?? ''}`,
          ...
          <div className="bg-panel rounded-xl shadow-xl w-full max-w-md">
            <div className="px-4 sm:px-6 py-4 border-b border-line-soft flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-fg mt-0.5">Log: {formatDate(createTaskFromLog.log_date, { month: 'long', day: 'numeric', year: 'numeric' })}</p>
              </div>
