...
## 📣 Notifications & comms
- **`wrap-anywhere` is applied on the inspection card only.** The same class of overflow almost certainly exists wherever else free text renders inside a grid or flex item - daily log entries, RFI questions, change order descriptions. Not swept broadly because one report is not evidence of a pattern the way 100 date call sites were; worth a pass if a second one turns up.
- **No route in the app uses the `delete` permission action.** `lib/permissions.ts` defines view/create/edit/delete and every gated DELETE checks `'edit'`. Either wire `delete` up across the API or drop it from the model - right now the roles screen offers a checkbox that governs nothing.
- **"Only the assigned person" as a per-event routing choice** *(offered, not taken - Sep 2026)*. Today a structural recipient always ADDS to the routed audience: naming somebody on an inspection tells them *and* everyone routed the event. A tester reasonably asked whether it should narrow instead. Two ways to build it, both deferred: a per-event toggle in "Who gets told" ("also tell the assigned person" vs "only the assigned person"), or a company-wide default. Not built because it is another switch to explain and nobody has yet said the current behaviour is too noisy - revisit if a company with ten office staff complains.
- **Per-project notification routing.** "Who gets told" is company-wide. A GC running six jobs may want the PM on job 3 to hear job 3's inspections and nobody else's. The rules table is keyed `(company_id, type)`; a nullable `project_id` and a most-specific-wins lookup would do it.
...
## 💰 Money
- **Re-pointing an invoice allocation would lift the estimate-removal restriction.** Removing an uploaded estimate is refused when one of its line items has a bill allocated against it, because `invoice_allocations` is `ON DELETE CASCADE` and the allocation would be destroyed silently. There is currently no screen that moves an allocation from one budget line to another, so the only way out is to unpick it on the invoice. A "move this allocation" control would make the refusal recoverable in place.
...
## 📱 Misc
- **Nine files still declare components inside components** (18 of them, ratcheted in `lib/__tests__/swipe-dismiss.ts`). Each rebuilds its whole subtree on every render of the component around it - mostly stateless cards, so the cost is wasted work and a lost scroll position rather than the visible blink #443 fixed on the task drawer. Hoist them file by file; the ratchet may only go down.
