...
## 📣 Notifications & comms
- **The Bids tab cannot reach a sub without an account** *(audited, not yet fixed - see PR #296)*. `bid_invitations` has no token and no email column; inviting only calls `notify()`, which reaches people who already have accounts. `/bid/<token>` resolves against `bid_invites` only. There is **no design reason for two bid systems, only age**: `bid_invitations` is migration **001** and assumes everyone has an account, `bid_requests`/`bid_invites` arrived at **024** precisely because that assumption was wrong, and nobody merged them. Fix by giving the Bids tab the same token mechanism rather than growing a third - the sub-facing `/bid/<token>` page already exists and works.
