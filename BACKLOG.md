...
## 📮 Invites - known limit, not yet fixed
- **Resend cannot re-invite somebody who already has an auth user.** The first invite creates a Supabase auth user; `inviteUserByEmail` then refuses a second call for that address ("already registered"), which `app/api/invite/route.ts` records as `emailSent: false`. So Resend on a pending invite is expected to report "Not resent" - honest now (#348), but still not doing the thing the button is named for. The fix is to stop leaning on `inviteUserByEmail` for the resend case: mint a fresh link with `auth.admin.generateLink({ type: 'invite', email })` and send it through `lib/email.ts`, which we control and which is already configured for SendGrid. Not done blind - it needs testing against a real Supabase, and the container cannot reach one.
