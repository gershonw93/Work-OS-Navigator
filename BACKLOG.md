...
## 🔑 Roles - open questions from the same review
- **Settings has a SECOND permission model.** `app/(dashboard)/settings/page.tsx` filters its tabs with hardcoded role strings (`isAdmin`, `isManager`, `isRestricted`) and never consults `settings_company` / `settings_team` / `settings_billing`, company role overrides, or per-user overrides. So a company that customises a role gets the customisation everywhere EXCEPT Settings. Same disease as #337: the rule exists in one place and not the others. The gating is currently correct by luck, not by construction.
- **The default tiers are coarse, and that is a product decision.** Field Supervisor is already walled off from all money by default (verified). But `office_staff` gets budget/invoices/payments FULL and `project_manager` gets budget FULL - so in practice it is: Admin, two near-admins, a field-only supervisor, and Worker. A GC evaluating "one place for the whole company" is likely to push back on the two near-admins, not on the supervisor. Changing the shipped defaults changes what existing users can see, so it needs a decision, not a guess.
- The deposit amount box showed `undefined-500` after a rejected submit (#341). Reported from testing. **I could not reproduce it or find a path in `payment-requests.tsx` that writes that string** - the component only ever sets `value: ''` or `e.target.value`, and production plus Supabase are both unreachable from the build container, so no live repro was possible. What the string IS, unambiguously, is `String(undefined) + '-500'`, so the fix goes after that shape wherever it can occur: new `toAmountInput()` in `lib/validate.ts` returns **empty** for null/undefined/NaN, where `String()` returns the literal words "undefined"/"null"/"NaN" into an editable field; applied to every unguarded money prefill on the payments and invoices pages. Also found by the new test rather than by reading: `money('undefined-500')` was refused as **"That is a negative amount"**, because the cleaner stripped the letters and left `-500` - this file's own sin, cleaning input until the check judges a laundered copy. `money()` now removes only real decorations (currency symbol, separators, whitespace) and then requires the remainder to BE a number. Third, the deposit form had no inline message at all - #339 named it and missed it - so the rejection landed in a banner at the top of the card with the offending value still in the box; it now judges the field with the same rules the server uses, shows the message under the field, and disables Request.
- Row-Level Security on every table, stated in a migration (#340). Supabase's advisor sent a CRITICAL alert: 8 public tables had RLS off, so the anon key - which ships in the browser bundle and is public by design - could read, edit or delete every row in them without touching the app or #337's guards. The one that mattered was `client_invoices`, which carries the portal share `token`: reading it handed you a working client-portal link for all 6 live invoices, and the portal shows invoices and payment history. Migration 092 also closed a second, quieter problem - of 74 tables in `public`, only 23 enabled RLS anywhere in these files; the other 51 were correct solely because somebody clicked a toggle in the dashboard, so a **fresh environment would have been born wide open**. Verified before writing rather than assumed: the browser reads exactly one table with the anon key (`projects`, not in the list), server layouts reach only `profiles`/`projects` as the signed-in user, and the unauthenticated client portal builds a service-role client - so nothing broke. Proven after: anon now sees 0 rows where 6 tokens exist, and the advisor's criticals are gone. A source guard fails the build if any future table forgets. Also pinned `bump_client_invoice_view`'s `search_path` (SECURITY INVOKER, so hygiene rather than a hole).
