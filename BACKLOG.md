...
## 🔓 Fail-open guard, found while documenting env vars (not fixed)
- **`CRON_SECRET` unset leaves `/api/cron/*` open.** `app/api/cron/compliance-reminders/route.ts:22` reads `if (!isVercelCron && secret && provided !== secret) return 401` - the `secret &&` means that with the variable unset the condition is false and the job **just runs** for anyone who requests it. Same fail-open shape as the activity feed in #342: a guard that only holds when an optional value happens to be set is not a guard. Low severity (it sends compliance reminder emails, so the worst case is reminder spam and a chewed-up SendGrid quota), which is why it is written down rather than patched blind: making it fail closed will 401 any *manual* trigger until `CRON_SECRET` is actually set in Vercel. Set the variable first, then flip the guard.
