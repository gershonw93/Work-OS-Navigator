...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'text', text: 'WHAT IT IS CALLED depends on the job. If the project has a client on it you are building for someone, so it is your CONTRACT VALUE - what you are charging them. If it has no client you are building to sell, and it is a SELLOUT - what it goes for. Same figure either way; only custom homes and fit-outs were being asked about a sellout they do not have.' },
      { type: 'text', text: 'MARKUP is a different number and lives on the Budget tab at every stage. Before a job is won it drives the client price on your proposal. After it is won that price is settled, so the full estimate bar goes away and a compact percent box stays - because on a cost-plus job the markup is still the fee you bill, and it is the same figure as the contractor fee rate on Payments & Escrow. It is hidden only on AIA jobs, where progress billing replaces it.' },
