...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  // ── Money: Budget ──────────────────────────────────────────────────────────
  {
    ...
    keywords: [
      ...
      'paint color', 'paint color', 'tile', 'flooring', 'cabinets', 'countertops', 'fixtures',
    ...
    blocks: [
      ...
      { type: 'steps', items: [
        ...
        'Add options where you have them. Each one takes a brand, a price, a link to the product page, and a PHOTO you drop or upload - a phone shot of the sample board, a screenshot off the supplier site. There is no color picker, because nobody eyeballs Sherwin Williams Alabaster off a color wheel. Got a whole fan deck? "Paste a list" takes one per line, and a #hex in that list still draws a swatch.',
      ...
      { type: 'tip', text: 'Rather than loading every option, put the manufacturer\'s color chart or product page in "Where they can browse the range". That link shows at the top of the selection on the client\'s side, so they go and look at the real thing and then write in what they picked.' },
      { type: 'text', text: 'Each selection hangs off a budget line - that is what makes the allowance mean something and where the cost lands when you order. The board suggests a line for each one and you change whatever is wrong before it saves; it never links silently, because a wrong link puts a client\'s upgrade on somebody else\'s line.' },
      { type: 'warn', text: 'You cannot mark a selection chosen or ordered until it has a budget line. An accepted selection is money, and money with nowhere to land is exactly what this board exists to prevent. If nothing fits, "Add a line for this" creates one named from the selection and seeded with its allowance.' },
      { type: 'tip', text: 'Link a selection to a line and the allowance fills itself in with whatever is left on that line, after the other selections already pointing at it. Link one to a line with nothing budgeted and it offers to set the line to the allowance instead - usually the line is the thing that was never filled in.' },
      { type: 'text', text: 'When the client has chosen and you are ready to buy, "Order it" records the supplier (from your Directory), the amount - already priced from what they picked - and the expected delivery. That books the cost against the budget line and the client\'s link shows it as Ordered.' },
  ...
  },
  {
    ...
    category: 'money',
    keywords: ['budget', 'line item', 'cost', 'estimate', 'committed', 'actual', 'interior', 'exterior', 'square footage', 'sq ft', 'bulk delete', 'select all', 'duplicate', 'hard cost', 'soft cost', 'category', 'categories', 'custom category', 'trade', 'craning', 'trusses', 'waterproofing', 'low voltage', 'sprinklers', 'demolition'],
  ...
  // ── Settings & Team ────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'steps', items: [
        ...
        'Use Sync customers, Sync subs (vendors), Sync bills, and Sync payments to push those records.',
        'Watch the Recent sync activity list to confirm what went over (and see any errors).',
      ] },
      { type: 'text', text: 'What each button sends: customers become QuickBooks Customers; subs become Vendors; approved or paid sub invoices become Bills (accounts payable); and client payments you have logged become Sales Receipts (money in). Bills and payments automatically create their vendor or customer in QuickBooks first if it is not there yet.' },
