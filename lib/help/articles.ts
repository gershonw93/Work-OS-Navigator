...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'text', text: 'MARKING ONE PAID records the money. Press Mark paid and the normal Record a client payment box opens with the invoice amount and number already filled in - put in the date and how they paid (check, ACH, wire) and save. That is what puts the money in Funds Received and your escrow balance, and what settles the invoice in QuickBooks. Change the amount before saving if they paid something different.' },
      ...
      { type: 'text', text: 'THE QB TICK on an invoice means the same thing it does on a payment: that record is in QuickBooks. An invoice raised before you connected QuickBooks will not have one until you press Sync client invoices in Settings.' },
  ...
  // ── Settings & Team ────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'text', text: 'MATCHING A RECORD IN QUICKBOOKS. Every pushed record carries the project name in its memo ("Hudson Yards Tower A - Draw 8") and a reference number like SN-9f84f2bd you can search for in QuickBooks. Hover the QB chip on a payment in SyteNav to see its reference. The "NO." column in QuickBooks lists is that reference - unless your QuickBooks company auto-numbers transactions, in which case the memo is your match. Records pushed before this existed can be brought up to the same format with UPDATE FORMATTING on the Settings card - it rewrites the memo and reference on the records already in QuickBooks, in place, without duplicating anything.' },
      { type: 'steps', items: [
