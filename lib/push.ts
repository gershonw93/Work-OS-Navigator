...
import { FCM_KEY_UNREADABLE, fcmReasonHelp } from './fcm'
...
// Lives in lib/pem.ts now, because the Android sender (lib/fcm.ts) reads its
// Google key through the same repair - a service-account key pasted into
// Vercel loses its line breaks exactly the way a .p8 does.
export { normalizePrivateKey } from './pem'
...
export function pushTestMessage(
  ...
  if (!r.error) return { ok: false, text: 'Nothing was sent, and the notification service gave no reason why.' }
  ...
  // WHOSE refusal it is. An Android phone's comes from Google (lib/fcm.ts
  // prefixes every one of its errors "FCM"), an iPhone's from Apple, and
  // somebody with one of each can get both in one string. Naming the wrong
  // company sends somebody to the wrong console.
  const parts = reason.split(', ')
  const google = parts.filter(p => /^FCM\b/.test(p) || p.includes(FCM_KEY_UNREADABLE)).length
  const help = google === parts.length
    ? fcmReasonHelp(reason) ?? apnsReasonHelp(reason)
    : apnsReasonHelp(reason) ?? fcmReasonHelp(reason)
  ...
  // what it did.
  const ours = reason.includes(KEY_UNREADABLE) || reason.includes(FCM_KEY_UNREADABLE)
  const lead = ours
    ? `SyteNav could not sign the request: ${reason}`
    : google === parts.length ? `Google refused it: ${reason}`
