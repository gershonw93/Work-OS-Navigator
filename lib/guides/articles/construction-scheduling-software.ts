...
export const guide: Guide = {
  ...
  blocks: [
    ...
    { type: 'p', text: 'A schedule line is a trade, a set of dates and optionally the sub who owns it - and a line with no sub attached is a placeholder, so a chain can point at "sheetrock, dates to be confirmed" before anybody is awarded. Dependencies point at a LINE rather than a trade, which is what lets electrical rough-in and electrical finish be waited on separately. A link can carry a few days of lag, and it can carry a progress gate: drywall does not start because framing is finished, it starts because framing is far enough along. Links are also suggested from a declared build order, but only where the dates you have already typed agree with it, and always as an offer you accept rather than something written for you - an unrecognised trade produces no suggestion at all rather than the nearest guess. A placeholder is not left behind when the work is awarded either: the new line appears as normal, and SyteNav asks whether it takes the placeholder\u2019s place - it asks rather than matching on the trade name, because a trade name is typed on both sides and a wrong merge would rewire another trade\u2019s chain onto the wrong sub with nothing on screen to show it.' },
    ...
    { type: 'p', text: 'Two notifications carry it, and neither sends itself: the sub whose dates moved is told, and the sub whose progress gate has just opened is told they are clear to start. Both go through a review screen that names who would be written to, and a line is only ever offered once, so nobody gets the same letter twice. There are three views over the same job - a month calendar, a timeline and a list. Only the calendar draws the job’s other dated things beside the schedule: tasks that are due, and inspections that are actually booked. The timeline and the list stay schedule-only, because they are the editor, and a row nothing can save does not belong in one. On a phone the month grid is dots, and tapping a day opens what is on it.' },
