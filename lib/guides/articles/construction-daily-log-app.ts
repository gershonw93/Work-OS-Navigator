...
export const guide: Guide = {
  ...
  blocks: [
    ...
    { type: 'p', text: 'Inspections run request to booking to result with the two dates kept separate. The request needs the inspection type and the date needed by. Whoever is booking it gets the numbers the job already knows - the issuing authority and inspector from your permits, plus any inspectors in your Directory - and the screen says plainly that SyteNav does not contact the inspector for you. Booking asks for the confirmed date, the window, who you spoke to and the confirmation number. Only booked inspections appear on calendars. What happens to those dates when a trade upstream runs late is its own subject. The sub presses "Work is ready" when the work is done, which is its own permission because saying the work is finished is a report from the site rather than office work. If an inspection is booked within two days and nobody has marked it ready, the people who asked for it and the people booking it are told. The inspector’s card is scanned afterwards and fills in the details, but nothing is marked passed or failed until a person says so.' },
  ...
  links: [
    { text: 'What happens to those dates when a trade upstream runs late', href: '/guides/construction-scheduling-software' },
