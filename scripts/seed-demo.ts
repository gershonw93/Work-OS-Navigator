/**
 * Seed a fully-populated demo account for SyteNav.
 *
 * Creates (or refreshes) the login  demo@sytenav.com  and fills 12 projects
 * with data on EVERY tab: plans/files, budget, subcontracts, invoices, client
 * payments, schedule, tasks, daily logs (+photos), time clock, RFIs, change
 * orders, permits, inspections, submittals, compliance, materials, equipment,
 * quotes/RFQs, team, and the activity feed.
 *
...
const DEMO_EMAIL = 'demo@sytenav.com'
const DEMO_PASSWORD = process.env.DEMO_PASSWORD || 'SyteNavDemo2026!'
...
// insert that tolerates a missing column (e.g. companies.added_by_company_id)
...
const TASK_TITLES = [
  'Order long-lead HVAC equipment', 'Confirm framing inspection date', 'Submit electrical panel cut sheet',
  'Coordinate concrete pour', 'Punch list walkthrough', 'Update as-built drawings',
  'Verify fire-stopping at penetrations', 'Schedule final grading', 'Review shop drawings for millwork',
...
const RFI_Q = [
  ...
  'Clarify concrete PSI for the loading dock slab.',
]
const CO_TITLES = [
  'Added recessed lighting in lobby', 'Upgrade to porcelain tile in restrooms',
  'Additional footing at soft soil area', 'Owner-requested millwork upgrade',
]
...
// ── main ────────────────────────────────────────────────────────────────────
async function main() {
  console.log('→ ensuring demo login', DEMO_EMAIL)
  // Find existing auth user by email (paginate a few pages, plenty for a demo db)
  ...
  if (userId) {
    ...
    const { data, error } = await db.auth.admin.createUser({ email: DEMO_EMAIL, password: DEMO_PASSWORD, email_confirm: true })
    if (error) throw error
    userId = data.user.id
    console.log('  created auth user', userId)
  }

  // Wipe any previous demo company (cascades projects + child rows)
  ...
  if (existingProfile?.company_id) {
    ...
    // vendors this demo company added to the directory (best-effort; ignore if column absent)
    await db.from('companies').delete().eq('added_by_company_id', existingProfile.company_id).then(() => {}, () => {})
    await db.from('companies').delete().eq('id', existingProfile.company_id) // cascades projects → all children
  }
  ...
  await db.from('profiles').upsert({
    id: userId, company_id: companyId, email: DEMO_EMAIL, full_name: 'Demo Admin',
    role: 'admin', phone: '(212) 555-0100',
  })
  console.log('  company + profile ready')

  // Directory vendors (subs / suppliers / inspector)
  const vendorIds: Record<string, string> = {}
  for (const v of VENDORS) {
    const id = rid()
    await insertSoft('companies', {
      id, name: v.name, type: v.type, trade: v.trade, contact_email: v.email,
      phone: v.phone, insurance_status: 'active', added_by_company_id: companyId,
    }, ['added_by_company_id'])
    vendorIds[v.name] = id
  }
  const subVendors = VENDORS.filter(v => v.type === 'subcontractor')
  console.log(`  ${VENDORS.length} directory vendors`)
  ...
  // Company equipment (+ some checked out to projects later)
  const EQUIP = [
  ...
  console.log(`  ${EQUIP.length} equipment items`)
  ...
  for (const proj of PROJECTS) {
    ...
    await insert('projects', {
      id: projectId, gc_company_id: companyId, name: proj.name,
      address: `${proj.addr}, ${proj.city}`, client: pick(CUSTOMERS, p).name,
      customer_id: pick(customerIds, p), start_date: ymd(start), end_date: ymd(end),
      type: proj.type, status: proj.status, latitude: proj.lat, longitude: proj.lng,
      lat: proj.lat, lng: proj.lng, interior_sqft: proj.iSq, exterior_sqft: proj.eSq,
      contractor_fee_pct: feePct,
    })

    // Team members (crew on this job)
    const teamCount = 3 + (p % 3)
    await insert('project_team_members', CREW.slice(0, teamCount).map(m => ({
      project_id: projectId, name: m.name, role: m.role, phone: m.phone,
      email: `${m.name.split(' ')[0].toLowerCase()}@sytenavdemo.com`, profile_id: null,
    })))

    // Plans / files
    const planTypes = ['architectural', 'structural', 'mep', 'civil'] as const
    await insert('project_plans', planTypes.slice(0, 3 + (p % 2)).map((t, i) => ({
      project_id: projectId, name: `${proj.name.split(' ')[0]}_${t}_set_R${i + 1}`,
      plan_type: t, file_url: SAMPLE_PDF,
    })))
    ...
    // Subcontracts (link 4-6 directory subs) + budget lines
    ...
    for (let s = 0; s < subCount; s++) {
      ...
      const row = await insert('subcontracts', {
        project_id: projectId, company_id: vendorIds[v.name], scope: `${v.trade} scope per plans`,
        trade: v.trade, contract_amount: amount, status: s === 0 ? 'completed' : 'active',
      })
      const scId = row[0].id
    ...
    // Budget lines (some linked to subs)
    for (let b = 0; b < BUDGET_LINES.length; b++) {
      ...
      await insert('budget_line_items', {
        project_id: projectId, cost_code: `${(b + 1) * 100}`, category: bl.cat, description: bl.desc,
        budgeted_amount: budgeted, committed_amount: linkedSub ? linkedSub.amount : round2(budgeted * 0.6),
        actual_amount: round2(budgeted * (0.2 + (b % 4) * 0.15)), sort_order: b,
        subcontract_id: linkedSub?.id ?? null, space_type: bl.space, progress_pct: (b * 8) % 100,
      })
    }

    // Invoices (varied statuses) against subs
    ...
    for (const sc of subcontractIds) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        await insert('invoices', {
          ...
          client_paid: status === 'paid' ? round2(amount * 0.5) : 0,
          escrow_paid: status === 'paid' ? round2(amount * 0.5) : 0,
        })
      }
    }

    // Client payments (draws)
    ...
    // Schedule items (subcontract date ranges) + a milestone
    let sIdx = 0
    for (const sc of subcontractIds) {
      await insert('schedule_items', {
        project_id: projectId, subcontract_id: sc.id,
        start_date: ymd(daysFromNow(-30 + sIdx * 10)), end_date: ymd(daysFromNow(-10 + sIdx * 10 + 8)),
      })
      sIdx++
    }
    await insert('schedule_items', {
      project_id: projectId, subcontract_id: null, label: 'Substantial Completion',
      color: '#C9F24A', start_date: ymd(end), end_date: ymd(end),
    })

    // Tasks
    await insert('project_tasks', TASK_TITLES.slice(0, 5 + (p % 4)).map((t, i) => ({
      project_id: projectId, title: t, description: 'Auto-seeded demo task.',
      due_date: ymd(daysFromNow(3 + i * 4)), priority: pick(['low', 'medium', 'high'], i),
      status: pick(['open', 'in_progress', 'open', 'done'], i),
      assigned_to_name: pick(CREW, i).name, created_by: 'Demo Admin',
      completed_at: i % 4 === 3 ? iso(daysFromNow(-2)) : null,
    })))
    ...
    for (let d = 0; d < 4; d++) {
      const logRow = await insert('daily_logs', {
        ...
        notes: pick([
          'Framing continued on the second floor. Deliveries on schedule.',
          'Rough plumbing inspection passed. Started electrical rough-in.',
          'Concrete pour for the rear slab completed. Cured overnight.',
          'Drywall hung on levels 1-2. Punch list started in unit A.',
        ], d),
      ...
      await insert('daily_log_photos', [0, 1].map(n => ({
        daily_log_id: logRow[0].id, photo_url: img(`${projectId}-${d}-${n}`), caption: `Site photo ${n + 1}`, category: 'progress',
      })))
    }
    ...
    for (let t = 0; t < 6; t++) {
      const m = pick(CREW, t)
      const inAt = daysFromNow(-1 - (t % 4))
      inAt.setHours(7, 0, 0, 0)
      const outAt = new Date(inAt); outAt.setHours(15, 30, 0, 0)
      await insert('time_entries', {
        project_id: projectId, profile_id: null, worker_name: m.name,
        clock_in_at: iso(inAt), clock_out_at: iso(outAt),
        clock_in_lat: proj.lat, clock_in_lng: proj.lng, clock_in_distance_m: 20 + t,
        clock_in_flagged: t % 5 === 4, clock_in_selfie_url: img(`clock-${projectId}-${t}`),
        approval_status: pick(['approved', 'pending', 'approved'], t),
        reviewed_by_name: t % 3 === 0 ? 'Demo Admin' : null,
      })
    }

    // RFIs
    await insert('rfis', RFI_Q.slice(0, 2 + (p % 3)).map((q, i) => ({
      project_id: projectId, question: q,
      response: i % 2 === 0 ? 'See revised detail A-501, issued in latest set.' : null,
      status: i % 2 === 0 ? 'answered' : 'open', created_by: userId,
    })))

    // Change orders
    await insert('change_orders', CO_TITLES.slice(0, 2 + (p % 2)).map((t, i) => ({
      project_id: projectId, subcontract_id: pick(subcontractIds, i).id, title: t,
      description: 'Owner-directed scope change.', amount: round2(3000 + i * 2500),
      reason: 'Owner request', requested_by_type: 'gc',
      status: pick(['approved', 'pending', 'approved', 'rejected'], i),
    })))

    // Permits
    await insert('permits', PERMIT_TYPES.slice(0, 3 + (p % 2)).map((t, i) => ({
      project_id: projectId, type: t, permit_number: `PB-${20000 + p * 100 + i}`,
      status: pick(['approved', 'pending', 'active', 'approved'], i),
      expiry_date: ymd(daysFromNow(120 + i * 30)),
    })))

    // Inspections
    await insert('inspections', INSPECTION_TYPES.slice(0, 3 + (p % 3)).map((t, i) => ({
      project_id: projectId, type: t, trade: t,
      status: pick(['passed', 'scheduled', 'requested', 'passed'], i),
      scheduled_date: ymd(daysFromNow(-10 + i * 7)),
    ...
    // Submittals
    await insert('submittals', SUBMITTAL_TITLES.slice(0, 3 + (p % 2)).map((t, i) => ({
      project_id: projectId, title: t, type: 'Product Data', trade: pick(subVendors, i).trade,
      spec_section: `0${9 + i} 00 00`, manufacturer: pick(['Kohler', 'Lutron', 'Andersen', 'Armstrong'], i),
      model_number: `M-${1000 + i}`, status: pick(['approved', 'pending', 'in_review'], i),
      submitted_by_company_id: pick(subVendors, i) ? vendorIds[pick(subVendors, i).name] : null,
      file_url: SAMPLE_PDF,
    })))

    // Compliance documents (for the subs on this job)
    for (let c = 0; c < subcontractIds.length; c++) {
      const sc = subcontractIds[c]
      await insert('compliance_documents', {
        company_id: sc.companyId, project_id: projectId,
        type: pick(['coi', 'license', 'w9', 'workers_comp'], c),
        status: pick(['approved', 'pending', 'expiring_soon', 'approved'], c),
        expiry_date: ymd(daysFromNow(30 + c * 40)), file_url: SAMPLE_PDF,
      })
    }
    ...
    await insert('material_purchases', (someLines ?? []).map((bl: any, i: number) => ({
      ...
      budget_line_id: i % 2 === 0 ? bl.id : null, store_name: pick(['Certified Lumber & Home Center', 'MetroTile Supply'], i),
      amount: round2(450 + i * 380), tax: round2((450 + i * 380) * 0.08875),
      purchase_date: ymd(daysFromNow(-7 - i)), category: bl.category, notes: 'Job materials',
      receipt_url: img(`receipt-${projectId}-${i}`), client_paid: i % 2 === 0,
      created_by: userId,
    })))

    // Equipment assignments (check out a couple items to this job on the first few projects)
    if (p <= EQUIP.length) {
      const eqId = equipIds[p - 1]
      await insert('equipment_assignments', {
        equipment_id: eqId, company_id: companyId, project_id: projectId,
        holder_name: pick(CREW, p).name, checked_out_at: iso(daysFromNow(-6)), note: 'On site',
        created_by: userId,
      })
      await db.from('equipment').update({ status: 'checked_out' }).eq('id', eqId)
    }

    // RFQ / quotes (bid request + invites + submissions + comparison)
    const brRow = await insert('bid_requests', {
      project_id: projectId, title: `${pick(subVendors, p).trade} package`, trade: pick(subVendors, p).trade,
      description: `${pick(subVendors, p).trade} for ${proj.name} per attached plans.`,
      due_date: ymd(daysFromNow(10)), status: 'open', created_by: userId,
    })
    ...
    const compId = rid()
    await insert('quote_comparisons', {
      id: compId, project_id: projectId, title: `${pick(subVendors, p).trade} package`,
      trade: pick(subVendors, p).trade, bid_request_id: brId, created_by: userId,
      requirements: 'Full scope per RFQ drawings.',
    })
    for (let q = 0; q < 3; q++) {
      const v = pick(subVendors, p + q)
      const inviteRow = await insert('bid_invites', {
        bid_request_id: brId, token: rid().replace(/-/g, ''), vendor_company_id: vendorIds[v.name],
        vendor_name: v.name, vendor_email: v.email, status: q < 2 ? 'submitted' : 'invited',
      })
      ...
      if (q < 2) {
        ...
        await insert('quotes', {
          comparison_id: compId, file_url: SAMPLE_PDF, file_name: `${v.name.split(' ')[0]}_Proposal.pdf`,
          vendor_name: v.name, total_amount: total, valid_until: ymd(daysFromNow(20)),
          scope_summary: `${v.trade} full scope per RFQ.`,
        })
      }
    }
    ...
    console.log(`  ✓ ${p}/${PROJECTS.length}  ${proj.name}`)
  }
