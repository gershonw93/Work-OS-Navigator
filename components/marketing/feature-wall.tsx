...
// The full feature wall as tabs, one tab per group.
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-6 py-14 sm:py-16">
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-8">
        {GROUPS.map((g, i) => (
          <button key={g.group} onClick={() => setActive(i)}
            className={cnTab(i === active)}>
            {g.group}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {group.items.map(f => (
          <div key={f.title} className="rounded-xl border border-line bg-panel p-5">
            <div className="h-10 w-10 rounded-lg bg-accent-tint flex items-center justify-center mb-3"><f.icon className="h-5 w-5 text-accent-fg" /></div>
...
function cnTab(active: boolean) {
  return [
    ...
    active ? 'bg-accent text-accent-ink border-accent' : 'bg-panel text-muted-fg border-line hover:text-ink hover:border-muted2',
  ].join(' ')
}
